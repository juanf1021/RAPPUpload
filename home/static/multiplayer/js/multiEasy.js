...
let principalTxt = document.getElementById("principal-text")
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  nonVisible(volver);
  nonVisible(document.getElementById('wait-image')); 
  ...
  var elements = document.getElementsByClassName("full-word");
  volver.addEventListener("click", () => {
    if (!volverBtnBool) {
      const container = document.querySelector('#wordContainerHtml');
      const numElements = container.querySelectorAll('*').length;
      //this will only display the wait image when theres no more than 2 element inside of the parent container
      try{
        if(numElements<=3){
          visible(document.getElementById('wait-image')); 
        }
      }catch(error){
        return 0;
      }
      ...
      for (var i = 0; i < elements.length; i++) {
        elements[i].style.margin = "0px 10px 30px 0px";
      }
    } else {
      document.getElementById("all-new").className = "";
      volverBtnBool = false;
      for (var i = 0; i < elements.length; i++) {
        elements[i].style.margin = "0px 30px 30px 30px";
      }
    }
  });
});
...
function usedWords(data, iterator, boolBtn) {
  nonVisible(document.getElementById('wait-image')); 
  let wordContainer = document.createElement("div");
  wordContainer.className = "full-word";
  if (!boolBtn) {
    wordContainer.style.margin = "0px 30px 30px 30px";
  } else {
    wordContainer.style.margin = "0px 10px 30px 0px";
  }
...
function playTimer() {
  ...
  if (count == 0) {
    ...
    if (count % 10 == 0) {
      wordIterator = wordIterator+10; 
      usedWords(datos, wordIterator, volverBtnBool);
      showWordList(datos, wordIterator);
      deleteUsedWords(wordIterator);
    }
    iterator++;
    count--;
...
function restartTimer() {
  ...
  restartValues();
  musicStart();
  changeInner(seconds, "PREPÁRATE");//!!hereeeeeee
  seconds.className = "preparate";
  changeInner(palabra, " ");
  isPlaying = true;
  visible(btnPlay);
...
function createDivWaitImg(){
  const newDiv = document.createElement('div');
  newDiv.setAttribute('id', 'wait-image');
  newDiv.setAttribute('class', 'image-container');

  const newImg = document.createElement('img');
  newImg.setAttribute('class', 'img-fluid');
  newImg.setAttribute('src', '/static/multiplayer/media/videos/rappx.png');
  newImg.setAttribute('alt', 'alternative');
  newDiv.appendChild(newImg);

  const newP = document.createElement('p');
  newP.textContent = '"Music is like magic. There\'s a certain feeling you get when you real and you spit and people are feelin\' your shit." - Eminem';
  newDiv.appendChild(newP);

  wordContainerHtml.appendChild(newDiv);
}
function musicStart() {
  var estallido = audios.options[audios.selectedIndex].id;
